...
- **Status bar disclaimer updated** to `READ ONLY -- EDUCATIONAL SIMULATION -- NO PHI` to flag that all patients are fictional.
...
- **JLV / Remote Data** (`openJlvInfo()`) — clicking it opens a small explanatory popup describing what JLV does in real CPRS (pulls in outside/non-VA records or records from a different VA facility). Explicitly marked "Not available in this simulation" — no real browser tab opens.
...
- **Two separate tour phases, plus a growing set of standalone sub-tutorial modules:**
  ...
  - `CHART_TOUR_STEPS` (17 steps) — runs inside an open chart; covers patient header, tab bar, the full personal-list-building workflow (`PERSONAL_LIST_TOUR_STEPS`, 7 steps, spliced in), Cover Sheet, Notes, Labs, Orders, Reports, and pre-rounding workflow
...
- **Tutorial tour steps:** content is placeholder-level; needs a full pass to write the actual teaching points the educator wants to highlight (this is intentional — content was deferred for a separate editing session)
...
- ~~**Notes tab — teach the Templates button.**~~ **Done.** Decorative Templates popup built + taught in `NOTES_TOOLS_TOUR_STEPS`. See "Notes tab — Templates, View menu / Custom View, and column fixes" above.
