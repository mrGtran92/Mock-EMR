...
- **PDMP Query — button behavior doesn't match the real two-click flow.** Real CPRS: click PDMP Query → button relabels (something like **Query Results**) → click again → a small dialog with a few prompts appears → confirming files a *pended* note. This simulation's `openPdmpQuery()` still just files the note directly on one click. User explicitly deferred fixing the actual behavior (only the tutorial text was corrected to describe the real flow, with a note that this sim simplifies it) — revisit if higher click-fidelity ever becomes worth building.
- **Remote Data button — placeholder only.** `openRemoteDataInfo()` opens a "Not yet detailed in this simulation" popup. User plans to supply what Remote Data actually does in a future session — once that arrives, flesh out `remote-data-dlg`'s content in `index.html` and consider adding a dedicated step to `HEADER_TOUR_STEPS`.
